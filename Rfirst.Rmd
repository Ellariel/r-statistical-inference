---
title: 'Central Limit Theorem: evidence by exponential distribution'
geometry: "left=2cm,right=1cm,top=1cm,bottom=0.5cm"
output:
...
So, the expected (theoretical) mean of exponential distribution is `r mu` and the average sample mean is `r round(sample_mean,3)`. We can see that they are so close, as the Central Limit Theorem predicted to.
...
paste(sigma, round(sample_sigma,3))
```
